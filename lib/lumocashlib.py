...
def is_valid_lumocash_address(address, network='mainnet'):
    ...
    lumocash_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid LumoCash address
        return False

    if (address_version != lumocash_version):
        return False
...
# shims 'til we can fix the lumocashd side
def SHIM_serialise_for_lumocashd(sentinel_hex):
    from models import MOTIOND_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = MOTIOND_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in lumocashd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # lumocashd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    lumocashd_hex = serialise(obj)
    return lumocashd_hex


# shims 'til we can fix the lumocashd side
def SHIM_deserialise_from_lumocashd(lumocashd_hex):
    from models import MOTIOND_GOVOBJ_TYPES

    # unpack
    obj = deserialise(lumocashd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('lumocash.conf')
