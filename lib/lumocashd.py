...
class LumoCashDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some lumocashd methods
    ...
    @classmethod
    def from_lumocash_conf(self, lumocash_dot_conf):
        from lumocash_config import LumoCashConfig
        config_text = LumoCashConfig.slurp_config_file(lumocash_dot_conf)
        creds = LumoCashConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = lumocashlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = lumocashlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = lumocashlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_lumocashd_proto_version_with_sentinel_ping)
