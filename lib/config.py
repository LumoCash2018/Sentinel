...
sentinel_cfg = LumoCashConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_lumocashd_proto_version_with_sentinel_ping = 70207


def get_lumocash_conf():
    home = os.environ.get('HOME')

    lumocash_conf = os.path.join(home, ".lumocashcore/lumocash.conf")
    if sys.platform == 'darwin':
        lumocash_conf = os.path.join(home, "Library/Application Support/LumoCashCore/lumocash.conf")

    lumocash_conf = sentinel_cfg.get('lumocash_conf', lumocash_conf)

    return lumocash_conf
...
lumocash_conf = get_lumocash_conf()
network = get_network()
db = get_db_conn()
