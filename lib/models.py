...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, lumocashd):
        golist = lumocashd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_lumocashd(lumocashd, item)
    ...
    @classmethod
    def import_gobject_from_lumocashd(self, lumocashd, rec):
        import decimal
        import lumocashlib
        import inflection
        ...
        # shim/lumocashd conversion
        object_hex = lumocashlib.SHIM_deserialise_from_lumocashd(object_hex)
        objects = lumocashlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from lumocashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from lumocashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from lumocashd - LumoCashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(lumocashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from lumocashd! %s" % e)
            govobj.vote_delete(lumocashd)
            return (govobj, None)
    ...
    def vote_delete(self, lumocashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(lumocashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, lumocashd, signal, outcome):
        import lumocashlib
        ...
        output = lumocashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = lumocashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(lumocashd, signal)

    def sync_network_vote(self, lumocashd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = lumocashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = MOTIOND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 lumocash addr, non-multisig
            if not lumocashlib.is_valid_lumocash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid LumoCash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (lumocashlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/LumoCashDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import lumocashlib
        obj_data = lumocashlib.SHIM_serialise_for_lumocashd(self.serialise())
    ...
    def prepare(self, lumocashd):
        try:
            object_hash = lumocashd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = MOTIOND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not lumocashlib.is_valid_lumocash_address(addr, config.network):
    ...
    def hash(self):
        import lumocashlib
        return lumocashlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = MOTIOND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, lumocashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - lumocashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, lumocashd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - lumocashd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, lumocashd):
        now = int(time.time())
        return (self.created_at < (now - lumocashd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, lumocashd):
        if self.is_expired(lumocashd):
            return False

        return True

    def is_deletable(self, lumocashd):
        if self.is_expired(lumocashd):
            return True
