...
class GovernanceClass(object):
    ...
    def vote(self, lumocashd, signal, outcome):
        return self.go.vote(lumocashd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, lumocashd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(lumocashd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(lumocashd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to lumocashd "gobject list" output
    ...
    def lumocashd_serialise(self):
        import lumocashlib
        return lumocashlib.SHIM_serialise_for_lumocashd(self.serialise())
