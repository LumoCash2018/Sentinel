...
def has_lumocash_conf():
    import config
    import io

    valid_lumocash_conf = False

    # ensure lumocash_conf exists & readable
    #
    # if not, print a message stating that LumoCash Core must be installed and
    # configured, including JSONRPC access in lumocash.conf
    try:
        f = io.open(config.lumocash_conf)
        valid_lumocash_conf = True
    except IOError as e:
        print(e)

    return valid_lumocash_conf
...
def main():
    ...
    if not has_lumocash_conf():
        print("LumoCashCore must be installed and configured, including JSONRPC access in lumocash.conf")
        sys.exit(1)
